...
export default {
    data() {
        return {          
            nameList: ["赵","钱","孙","李","周","吴","郑","王","冯","陈","褚","卫","蒋","沈","韩","杨","朱","秦","尤",
    ...
    mounted() {
            ...
            series: [   //系列列表
                {
                    name: '销量1',
                    type: 'bar',
                    data: [70,12,45,6,30,12]
                },
                {
                    name: '销量2',
                    type: 'bar',
                    data: [10,18,15,6,10,33]
                },
                {
                    name: '销量3',
                    type: 'bar',
        ...
        var myChiart2 = this.echarts.init(document.getElementById('charts2'));
        myChiart2.setOption({   //配置图表参数
            title: {
                text: '',
            },
            series: {   //系列列表
    ...
    methods: {
        getData() {
            ...
            this.nameList.sort(function() {
                return Math.random() - 0.5;
            });

            for(var i = 0; i < 50; i++) {
                var makeUpName = this.nameList[0] + this.nameList[1];
                this.legendData.push(makeUpName);
                this.seriesData.push({name: makeUpName, value: Math.ceil(Math.random()*100000)});
            ...
            for(var i = 0; i < this.legendData.length; i++) {
                if(i < 3) {
                    this.selected[this.legendData[i]] = true;
                }else {
                    this.selected[this.legendData[i]] = false;
                }
            }
